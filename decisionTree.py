...
class DecisionTree:
    def __init__(
        self, min_samples_split=2, max_depth=100, n_features=None, random_state=None
    ):
    ...
    def fit(self, X, y, depth=0):
        ...
        # If n_features wasn't provided, default to using all features
        if self.n_features is None:
            self.n_features = X.shape[1]
        else:
            # Ensure it doesn't exceed total features
            self.n_features = min(self.n_features, X.shape[1])
        self.feature_importances_ = np.zeros(X.shape[1])
        print(
            f"Fitting tree with X shape: {X.shape}, y shape: {y.shape}, n_features: {self.n_features}"
        )
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        feat_idx = np.random.choice(X.shape[1], self.n_features, replace=False)
    ...
    def best_split(self, X, y, feat_idx):
        ...
        for feature in feat_idx:
            ...
            # Candidate splits are midpoints between unique sorted values
